...
function DaySelector({ days, selected, onSelect }) {
  ...
  const cellW = railW ? (railW - gap * 6) / 7 : 46;
  // land on the Monday of the selected day's week; free dragging from there
  const weekKey = React.useMemo(() => {
    const c = FYZIO.keyToDate(selected);
    if (!c) return selected;
    const m = new Date(c.getFullYear(), c.getMonth(), c.getDate() - ((c.getDay() + 6) % 7));
    return m.toDateString();
  }, [selected]);
  React.useEffect(() => {
    const el = railRef.current; if (!el || !railW) return;
    const cell = el.querySelector(`[data-day="${weekKey}"]`);
  ...
  const monthLabel = (() => {
    ...
    const c = days.find((x) => x.key === selected) || days[0];
    const mon = new Date(c.date.getFullYear(), c.date.getMonth(), c.date.getDate() - ((c.date.getDay() + 6) % 7));
    const sun = new Date(mon.getFullYear(), mon.getMonth(), mon.getDate() + 6);
    if (mon.getMonth() === sun.getMonth()) return `${cap(M[mon.getMonth()])} ${sun.getFullYear()}`;
    return `${cap(M[mon.getMonth()])} – ${cap(M[sun.getMonth()])} ${sun.getFullYear()}`;
  })();

  // a dot means "training day". Rest days carry no dot. Today is marked by the number.
  const dot = (d, isSel) => {
    if (d.notStarted || !d.isTraining) return null;
    const hollow = d.status === "missed";
    const col = isSel ? "rgba(255,255,255,0.85)" : d.status === "done" ? "var(--ok)" : "var(--accent)";
    return <span style={{ width: hollow ? 6 : 5, height: hollow ? 6 : 5, borderRadius: "50%",
      background: hollow ? "transparent" : col,
      border: hollow ? `1.5px solid ${isSel ? "rgba(255,255,255,0.85)" : "var(--faint)"}` : "none" }} />;
  };

  return (
    <div style={{ userSelect: "none" }}>
      <div style={{ padding: "0 20px 8px", fontSize: 12.5, fontWeight: 650, color: "var(--muted)" }}>{monthLabel}</div>
      <div ref={railRef} className="fz-rail" style={{ display: "flex", gap, overflowX: "auto", padding: "2px 0 4px", margin: "0 18px",
        scrollbarWidth: "none", WebkitOverflowScrolling: "touch", scrollSnapType: "x mandatory", touchAction: "pan-x" }}>
        {days.map((d) => {
          const isSel = d.key === selected;
          const isToday = d.status === "today";
          const snap = { scrollSnapAlign: "start", flexShrink: 0, width: cellW };
          const dayAttr = d.date.toDateString();
          if (style === "circles") {
            return (
              <button key={d.key} data-day={dayAttr} data-sel={isSel || undefined} onClick={() => onSelect(d.key)}
                style={{ ...snap, border: "none", background: "none", cursor: "pointer", fontFamily: "inherit",
                  display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: 0 }}>
                <span style={{ fontSize: 11.5, fontWeight: 600, color: isSel ? "var(--accent)" : "var(--muted)" }}>{d.dow}</span>
                <span style={{ width: 42, height: 42, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 16, fontWeight: isToday ? 800 : 650,
                ...
                <span style={{ height: 10, display: "flex", alignItems: "center" }}>{dot(d, false)}</span>
              </button>);
          ...
          if (style === "minimal") {
            return (
              <button key={d.key} data-day={dayAttr} data-sel={isSel || undefined} onClick={() => onSelect(d.key)}
                style={{ ...snap, border: "none", cursor: "pointer", fontFamily: "inherit",
                ...
                <span style={{ fontSize: 18, fontWeight: isToday ? 800 : 700 }}>{d.num}</span>
                <span style={{ height: 5, display: "flex", alignItems: "center" }}>{dot(d, isSel)}</span>
              </button>);
          ...
          return (
            <button key={d.key} data-day={dayAttr} data-sel={isSel || undefined} onClick={() => onSelect(d.key)}
              style={{ ...snap, border: `1.5px solid ${isSel ? "var(--accent)" : "var(--line)"}`,
              ...
              <span style={{ fontSize: 18, fontWeight: isToday ? 800 : 700, color: isSel ? "var(--accent-ink)" : isToday ? "var(--accent)" : "var(--ink)" }}>{d.num}</span>
              <span style={{ height: 13, display: "flex", alignItems: "center" }}>{dot(d, false)}</span>
            </button>);
...
// ── Check button: tap = +1 series, long press = reset to 0 ──
function useSeriesPress(onAdd, onReset) {
  const timer = React.useRef(null);
  const longFired = React.useRef(false);
  const clear = () => { clearTimeout(timer.current); timer.current = null; };
  return {
    onPointerDown: (e) => { e.stopPropagation(); longFired.current = false;
      timer.current = setTimeout(() => { longFired.current = true; onReset?.(); }, 520); },
    onPointerUp: (e) => { e.stopPropagation(); clear(); if (!longFired.current) onAdd(); },
    onPointerCancel: (e) => { e.stopPropagation(); clear(); },
    onPointerLeave: (e) => { e.stopPropagation(); clear(); },
    onClick: (e) => e.stopPropagation() };

}

// ── Exercise card ────────────────────────────────────────────
function ExerciseCard({ ex, done, seriesDone, onAddSeries, onOpen, tour, onMarkDone, onUndo, readOnly }) {
  const t = useT();
  ...
  const onPtrUp = () => { if (!swipe.current.active) return; swipe.current.active = false; const dx = swipe.current.dx; setOffset(0); if (!swipe.current.moved) { onOpen(); return; } if (readOnly) return; if (dx < -60) { onMarkDone?.(); } else if (dx > 60 && done) { onUndo?.(); } };
  ...
  return (
    <div onPointerDown={onPtrDown} onPointerMove={onPtrMove} onPointerUp={onPtrUp} onPointerCancel={onPtrUp}
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        <div title={ex.name} style={{ fontSize: d === "compact" ? 17 : 18.5, fontWeight: 720,
      ...
      {readOnly ?
      <div role="img" aria-label={done ? "Odcvičené" : "Neodcvičené"}
        style={{ alignSelf: "center", flexShrink: 0, width: 46, height: 46,
          display: "flex", alignItems: "center", justifyContent: "center" }}>
        <Icon name="check" size={22} stroke={done ? "var(--ok)" : "var(--line)"} sw={2.4} />
      </div> :
      <button {...checkPress}
        aria-label={done ? "Vynulovať série" : "Pripočítať sériu"}
        ...
        <span style={{ fontSize: 26, fontWeight: 400, lineHeight: 1, color: "var(--ink)", marginTop: -2 }}>+</span>}
      </button>}
    </div>);
          ...
          return (
            <button key={z.label} onClick={() => onChange(z.mid)}
              style={{ flex: 1, border: "none", cursor: "pointer", fontFamily: "inherit",
                borderRadius: 12, padding: "16px 4px 14px",
...
function PainCheckIn({ value, saved, onChange, onSave, bare }) {
    ...
    <React.Fragment>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }}>
        {Array.from({ length: 11 }).map((_, n) => {
          ...
          return (
            <button key={n} onClick={() => onChange(n)} style={{
              width: "calc((100% - 40px) / 6)", height: 44, borderRadius: 6, cursor: "pointer",
      ...
      <div style={{ marginTop: 16 }}>
        {saved ? (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 7,
            height: 48, borderRadius: 6, background: "var(--ok-wash)",
          ...
          <button onClick={onSave} disabled={value == null} style={{ width: "100%", height: 48, border: "none",
            borderRadius: 6, cursor: value == null ? "default" : "pointer", fontFamily: "inherit",
  ...
  if (bare) return body;
  return (
    <div style={{ background: "#fff", borderRadius: 12, padding: 20, border: "1px solid var(--line)",
...
function ReminderCard() {
  ...
  return (
    <div style={{ background: "var(--accent-ink-bg)", borderRadius: 12, padding: 18, color: "#fff",
...
function RestState() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ background: "#fff", borderRadius: 12, padding: "26px 22px",
        border: "1px solid var(--line)" }}>
        <div style={{ fontSize: 19, fontWeight: 750, color: "var(--ink)", letterSpacing: -0.4, marginBottom: 7 }}>Oddýchnite si</div>
        <div style={{ fontSize: 14.5, color: "var(--muted)", lineHeight: 1.5, textWrap: "pretty" }}>
          Dnes nemáte naplánované žiadne cviky. Doprajte telu čas na regeneráciu — pomáha to hojeniu aj výkonu.
        </div>
      </div>
      <div style={{ background: "#fff", borderRadius: 12, padding: "16px 18px", border: "1px solid var(--line)" }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink)" }}>Ľahký pohyb je vítaný</div>
        <div style={{ fontSize: 13.5, color: "var(--muted)", marginTop: 4, lineHeight: 1.45, textWrap: "pretty" }}>Krátka prechádzka alebo jemné strečovanie udrží telo v pohybe.</div>
      </div>
    </div>
  );
}
...
function CompletedState({ count }) {
  return (
    <div style={{ background: "var(--ok-wash)", borderRadius: 12, padding: "18px 20px", border: "1px solid var(--ok-line)" }}>
...
function FocusHero({ exs, series, onOpenEx }) {
  ...
  return (
    <div style={{ background: "var(--accent-ink-bg)", borderRadius: 12, padding: 20, color: "#fff",
      ...
      {!allDone && (
        <button onClick={() => onOpenEx(next.id)} style={{ marginTop: 16, width: "100%", border: "none", cursor: "pointer",
          fontFamily: "inherit", background: "#fff", color: "var(--accent-ink)", borderRadius: 6, padding: "14px",
...
function ExerciseRow({ ex, done, seriesDone, onAddSeries, onUndo, onOpen, readOnly }) {
  const partial = seriesDone > 0 && !done;
  const checkPress = useSeriesPress(onAddSeries, onUndo);
  return (
    <div onClick={onOpen} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px",
      ...
      {readOnly ?
      <div role="img" aria-label={done ? "Odcvičené" : "Neodcvičené"} style={{ flexShrink: 0, width: 38, height: 38,
        display: "flex", alignItems: "center", justifyContent: "center" }}>
        <Icon name="check" size={17} stroke={done ? "var(--ok)" : "var(--line)"} sw={2.4} />
      </div> :
      <button {...checkPress}
        style={{ flexShrink: 0, width: 38, height: 38, borderRadius: 12,
        cursor: "pointer", padding: 0, border: "1.5px solid var(--line)",
        backgroundColor: srBg(seriesDone, ex.sets),
        display: "flex", alignItems: "center", justifyContent: "center", transition: "background-color .2s ease" }}>
        {done ?
...
function DailyPainCard({ value, onOpen }) {
  ...
  return (
    <button onClick={onOpen} style={{ width: "100%", background: "#fff", borderRadius: 12,
  ...
  return (
    <div className="fz-fade" style={{ position: "relative", height: "100%", display: "flex", flexDirection: "column",
      ...
      <div style={{ flexShrink: 0, padding: "52px 18px 10px" }}>
        <button onClick={onClose} aria-label="Späť" style={{ width: 40, height: 40, borderRadius: 6,
      ...
      <div style={{ flexShrink: 0, padding: "12px 22px 30px" }}>
        <button onClick={onSave} disabled={value == null} style={{ width: "100%", border: "none",
          borderRadius: 12, padding: "17px", fontSize: 16.5, fontWeight: 680, fontFamily: "inherit",
...
function AssessmentCard({ onOpen }) {
  const { ref, pressed, handlers } = usePress(onOpen);
  return (
    <div ref={ref} {...handlers} className="fz-press" data-pressed={pressed ? "1" : "0"} role="button" tabIndex={0}
      style={{ width: "100%", cursor: "pointer", borderRadius: 12, padding: "14px 16px",
        background: "var(--info-wash)", border: "1px solid var(--info-line)", color: "var(--info-ink)",
        display: "flex", alignItems: "center", gap: 12 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", opacity: 0.85 }}>Hodnotiaci nástroj</div>
        <div style={{ fontSize: 14.5, fontWeight: 650, marginTop: 3, lineHeight: 1.35, textWrap: "pretty" }}>Závažnosť tendinopatie – Achillova šľacha (TENDINS-A) · ~5 minút</div>
      </div>
      <Icon name="chevR" size={18} stroke="var(--info-ink)" />
    </div>
  );
}
...
function HomeScreen({ selectedDay, onSelectDay, series, onAddSeries, onOpenEx, onMarkDone, onUndo, onOpenPain, onOpenCelebrate, program, onOpenPrograms, pain, painSaved, onPainChange, onPainSave, assessDone, onOpenAssessment }) {
  ...
  const weekDays = React.useMemo(() => FYZIO.dayRange(selectedDay, 3, 3, program || FYZIO.program), [selectedDay, program]);
  const day = weekDays.find((d) => d.key === selectedDay) || weekDays[4];
  const isToday = day.status === "today";
  const isRest = day.status === "rest" || isToday && !FYZIO.isTrainingToday(program || FYZIO.program);
  const isExerciseDay = !isRest;
  const isDone = day.status === "done";
  const readOnly = !isToday;
  const exs = FYZIO.exercises;
  // past days show what was actually completed; future days show the plan untouched
  const seriesOf = (e) => isToday ? series[e.id] || 0 : day.status === "done" ? e.sets : 0;
  ...
  const allDone = doneCount === exs.length;
  const prog = program || FYZIO.program;
  const isPaused = FYZIO.pState(prog) === "paused";
  const notStarted = !!day.notStarted;
  const canSwitch = FYZIO.homePrograms().length > 1;
  const th = FYZIO.therapists.find((x) => x.id === prog.therapistId);

  const renderCard = (ex, first) => layout === "zoznam" ?
  <ExerciseRow key={ex.id} ex={ex} done={isExDone(ex)} seriesDone={seriesOf(ex)} readOnly={readOnly}
    onAddSeries={() => onAddSeries(ex.id)} onUndo={() => onUndo?.(ex.id)} onOpen={() => onOpenEx(ex.id)} /> :
  <ExerciseCard key={ex.id} ex={ex} done={isExDone(ex)} seriesDone={seriesOf(ex)} readOnly={readOnly}
    ...
    <div>
      ...
      <div style={{ padding: "8px 20px 14px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
          <button onClick={canSwitch ? onOpenPrograms : undefined} data-tour="program"
            aria-label={prog.name}
            style={{ flex: 1, minWidth: 0, border: "none", background: "none",
            cursor: canSwitch ? "pointer" : "default", fontFamily: "inherit", textAlign: "left", padding: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              <span aria-hidden="true" title={prog.name} style={{ minWidth: 0, fontSize: 24, fontWeight: 750, color: "var(--ink)",
                letterSpacing: -0.4, lineHeight: 1.2,
                whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{prog.name}</span>
              {canSwitch &&
            ...
            <div style={{ fontSize: 13.5, color: "var(--muted)", marginTop: 4, fontWeight: 500 }}>{prog.phase} · {prog.week || FYZIO.program.weekLabel}</div>
          </button>
          {!isRest && !isPaused && !notStarted &&
          <div role="img" aria-label={`${doneCount} zo ${exs.length} cvikov`} style={{ flexShrink: 0 }}>
          <ProgressRing value={doneCount / exs.length} size={52} sw={5}>
            <span style={{ fontSize: 13, fontWeight: 750, color: "var(--ink)", fontVariantNumeric: "tabular-nums" }}>
      ...
      {/* Day selector */}
      <div data-tour="days">
        {!isPaused && <DaySelector days={weekDays} selected={day.key} onSelect={onSelectDay} />}
      </div>

      {/* Body */}
      <div style={{ padding: "16px 20px 8px", display: "flex", flexDirection: "column", gap: 14,
        paddingBottom: "calc(104px + max(env(safe-area-inset-bottom), 16px))" }}>
        {isToday && FYZIO.assessment.due && !assessDone && !isPaused && <AssessmentCard onOpen={onOpenAssessment} />}
        {isPaused &&
        <div style={{ background: "var(--warn-wash)", borderRadius: 12, border: "1px solid var(--line)", padding: "20px 20px 22px" }}>
          <div style={{ fontSize: 17, fontWeight: 720, color: "var(--warn-ink)", letterSpacing: -0.3, marginBottom: 7 }}>Program je pozastavený</div>
          <div style={{ fontSize: 14.5, color: "var(--warn-ink)", lineHeight: 1.5, textWrap: "pretty", opacity: 0.85 }}>
            Program dočasne pozastavil váš fyzioterapeut.
          </div>
        </div>}
        {!isPaused && notStarted &&
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid var(--line)", padding: "20px" }}>
          <div style={{ fontSize: 14.5, color: "var(--muted)", lineHeight: 1.5 }}>Program v tento deň ešte nebežal.</div>
        </div>}
        {!isPaused && !notStarted && isExerciseDay && (
          <React.Fragment>
            {layout === "fokus" && <FocusHero exs={exs} series={series} onOpenEx={onOpenEx} />}
            <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
              <span style={{ fontSize: 18, fontWeight: 720, color: "var(--ink)" }}>{isToday ? "Dnešný plán" : "Plán dňa"}</span>
              {readOnly &&
              <span style={{ fontSize: 13, fontWeight: 550, color: "var(--muted)" }}>len na čítanie</span>}
            </div>
            {allDone && isToday && (
              <div style={{ width: "100%",
                background: "var(--accent-wash)", border: "1px solid var(--accent-soft2)", borderRadius: 12,
            ...
            {/* Active exercises */}
            {readOnly && day.status === "done" && <CompletedState count={exs.length} />}
            {(readOnly ? day.status === "done" ? [] : exs : activeExs).map((ex, i) => renderCard(ex, i === 0))}
            ...
            {doneExs.length > 0 && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <button onClick={() => setShowDone((v) => !v)} style={{ display: "flex", alignItems: "center", gap: 7,
                  ...
                  <span style={{ fontSize: 14, fontWeight: 500, color: "var(--muted)" }}>· {doneExs.length}</span>
